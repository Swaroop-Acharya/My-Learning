...
const totalCountries = countries.map((country) => country.name).length;

const totalPopulation = countries.reduce(
  (sum, country) => sum + country.population,
  0
);
function totalCountryDisplay() {
  const p = document.querySelector(".subtitle");
  p.textContent = `Currently, we have ${totalCountries} countries`;
}
totalCountryDisplay();
...
const defaultTitle = `10 most in populated countries the world`;
const changeTitle = `10 most Spokend Languages the world`;
graphTitle.textContent = defaultTitle;
function graphTitleHandler(title) {
  if (title === defaultTitle) {
    graphTitle.textContent = defaultTitle;
  } else {
    graphTitle.textContent = changeTitle;
  }
}

function getSortedPopulation(countries) {
...
function getSortedLanguages(countries) {
  const languageArr = countries
    .map((ele) => ele.languages)
    .reduce((acc, curArr) => acc.concat(curArr), []);
  console.log(languageArr);
  const uniqueLang = new Set(languageArr);
  const arrofObj = [];

  for (const lang of uniqueLang) {
    const sameLangArr = languageArr.filter((l) => l === lang);
    arrofObj.push({
      name: lang,
      population: sameLangArr.length,
    });
  }
  const sortedLanguageArray = arrofObj
    .sort((a, b) => b.population - a.population)
    .filter((_, index) => index < 11);
  console.log(sortedLanguageArray);
  return sortedLanguageArray;
}

function graphBuilder(arr,totalCount) {
  arr.forEach((country) => {
    ...
    populationPara.textContent = ` ${country.population}`;
    bar.style.width = `${(country.population / totalCount) * 100}%`;
    bar.style.height = `25px`;
    mainDiv.classList.add("flexbox");
    barContainer.classList.add("barDiv");
    nameContainer.classList.add("nameContainer");
    populationContainer.classList.add("populationContainer");
    nameContainer.appendChild(countryName);
    populationContainer.appendChild(populationPara);
    barContainer.appendChild(bar);
    mainDiv.appendChild(nameContainer);
    mainDiv.appendChild(barContainer);
    mainDiv.appendChild(populationContainer);
...
function setupGraph(buttonElement, dataFunction, titleHandler, totalValue) {
  buttonElement.addEventListener("click", () => {
    ...
    const sortedDataArr = dataFunction(countries);
    console.log(sortedDataArr);
    barGraphContainer.innerHTML = '';
    graphBuilder(sortedDataArr, totalValue);
  });
}
document.addEventListener('DOMContentLoaded', e => {
  graphTitleHandler(defaultTitle);
  const sortedPopulationArr = getSortedPopulation(countries);
  console.log(sortedPopulationArr)
  barGraphContainer.innerHTML=''
  graphBuilder(sortedPopulationArr,totalPopulation);
  setupGraph(populationBtn, getSortedPopulation, defaultTitle, totalPopulation);
  setupGraph(languageBtn, getSortedLanguages, changeTitle, totalCountries);
});
