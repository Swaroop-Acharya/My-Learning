...
// const arr=["Swroop","Jhon cena","Beth","Morales","Lost"]
...
// let B=new Set(b)
// let newC =  a.filter((ele)=>B.has(ele))
// let result = new Set(newC)
// console.log(result)
// // Find a with b

// let B1=new Set(b)
// let newC1 =  a.filter((ele)=>!B1.has(ele))
// let result1= new Set(newC1)
// console.log(result1)
...
function mostSpokenLanguages(countries, count) {
  ...
  const langSet = new Set(resultArr);

  const solutionArr = [];

  for (const lan of langSet) {
    const filteredArr = resultArr.filter((lang) => lang === lan);
    // console.log(filteredArr)
    solutionArr.push({ [lan]: filteredArr.length });
  }
  return solutionArr
    .sort((a, b) => {
    ...
    }).filter((_, index) => index < count); // Use filter to slice
